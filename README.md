...
![密集会话：整图态聚合徽标 → 点击放大 → 标签补齐 → 点开失败详情](assets/maze-zoom.gif)

- **数据轨道**——每一步的工具调用密度、Token 脉冲（缓存背景 + 未缓存输入/推理/输出增量柱）、上下文压力曲线（70%/90% 阈值线、压缩事件「⌄−N%」标注，悬停看压缩前后真值）：

![轨道悬停：Token 分层数字 → 上下文占用 → 压缩事件前后对比](assets/maze-tracks.gif)
...
![对比：同一任务的两次真实跑 → 支路盘点按轮次列差额 → 点行缩放到该轮](assets/maze-compare.gif)
...
- **行为信号**（诊断层第 2 项，失败恢复链旁的块）：每条带严重度（高/中/低/信息）、一句话依据（含具体数字）和涉及调用数——失败后原样重试、同轮重复调用（含重复读取子标签）、循环、工具失败、慢调用（≥120 秒）、工具集中度（赫芬达尔指数）、上下文骤升/骤降（骤降落在压缩事件附近标「压缩」）、上下文峰值、压缩发生、待办陈旧、换策略恢复。阈值按本机 202 份会话校准（「中」落在最差的 15%~20% 会话，「高」落在最差的 3%~5%），常量在 `verdict.js` 的 `ANALYSIS_RULES.SIGNALS`；各信号独立计数。点一条淡化其他节点并定位到该段第一个调用（再点或按 Esc 取消，与其他过滤叠加）；没有信号时明说「本场没有触发任何信号」。code 模式本版只统计外层 run_code 调用，方法说明注明。上下文占用（峰值信号、上下文压力轨道、摘要卡与头部指标条）按每次请求当时的模型换算窗口：上传日志读 `request/context`，实时页签按逐请求模型查表；轨道纵轴是占用百分比，标题标注中途切过的窗口和因窗口值不可信而略过的样本。
