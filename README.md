...
- **行为信号**（诊断层第 2 项，失败恢复链旁的块）：每条带严重度（高/中/低/信息）、一句话依据（含具体数字）和涉及调用数——失败后原样重试、同轮重复调用（含重复读取子标签）、循环、工具失败、慢调用（≥120 秒）、工具集中度（赫芬达尔指数）、上下文骤升/骤降（骤降落在压缩事件附近标「压缩」）、上下文峰值、压缩发生、待办陈旧、换策略恢复。阈值按本机 202 份会话校准（「中」落在最差的 15%~20% 会话，「高」落在最差的 3%~5%），常量在 `verdict.js` 的 `ANALYSIS_RULES.SIGNALS`；各信号独立计数。点一条淡化其他节点并定位到该段第一个调用；没有信号时明说「本场没有触发任何信号」。code 模式会话的同轮重复率天然偏高，图例注明。
