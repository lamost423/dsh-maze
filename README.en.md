...
- **Outcome & evidence** (diagnosis layer item 1; the first block of the analysis section): six cells — task / tests / build / lint / artifacts / human check — plus one overall badge (completed / partial / unverified), all read from log evidence rather than the agent's own claims. Tests/build/lint are recognized by **command-position** regexes (JS/TS, Python, Rust, Go, Swift in this first cut; an npm/pnpm/yarn `check` script counts as tests; `cat vitest.config.ts` or `grep pytest` do not count), one command can count for several kinds; passed = the call carried no error flag and the **last line** of its output has no non-zero exit code; each category is judged by its last run, with earlier commands that failed on their last run noted in small print ("the same command" is the matched segment — a different `cd` prefix, redirection or `&& echo PASS` decoration does not make it a new one); a background job counts by the exit code on its later `job_output` last line and is left out (and noted) when none arrives; click a cell to locate the step of the last run and open that step's details. Artifacts = files touched by successful write/edit/patch calls, deduplicated (expand the list in the cell); task done = the last `turn/end` is not error/aborted/interrupted/blocked and the last step is an answer; the human check only asks whether a human message followed the final answer — content is never interpreted. With no verification command at all the badge reads a neutral "unverified"; code-mode sessions (real tool calls dispatched inside `run_code` scripts) are not recognized yet and the block says so. Rules live in `ANALYSIS_RULES.VALIDATION` in `verdict.js`.
- **Behavior signals** (diagnosis layer item 2; the block beside the failure recovery chains): each row carries a severity (high / medium / low / info), a one-line rationale with the actual numbers, and the number of calls involved — identical retry after failure, same-turn repeats (with a repeated-reads sub-tag), loops, tool failures, slow calls (≥120 s), tool concentration (Herfindahl index), context jumps/drops (a drop next to a compaction event is tagged as such), context peak, compaction, stale todo reminders, adaptive recovery. Thresholds were calibrated on 202 local sessions ("medium" lands in the worst 15–20% of sessions, "high" in the worst 3–5%) and live in `ANALYSIS_RULES.SIGNALS` in `verdict.js`; signals count independently. Click a row to dim everything else and locate the first call of that stretch (click again or press Esc to clear; stacks with the other filters); with nothing fired the block says so instead of hiding. In this version code-mode sessions count only the outer run_code calls, as the method note says. Context use (the peak signal, the context-pressure track, summary card and header strip) is converted with the window of the model in use at each request: uploaded logs read `request/context`, the live tab looks per-request models up in the table; the track's axis is a percentage, and its caption notes mid-session window switches and samples skipped because their declared window was untrustworthy.
