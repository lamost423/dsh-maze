...
- **Outcome & evidence** (diagnosis layer item 1; the first block of the analysis section): six cells — task / tests / build / lint / artifacts / human check — plus one overall badge (completed / partial / unverified), all read from log evidence rather than the agent's own claims. Tests/build/lint are recognized by **command-position** regexes (JS/TS, Python, Rust, Go, Swift in this first cut; `cat vitest.config.ts` or `grep pytest` do not count), one command can count for several kinds; passed = the call carried no error flag and the **last line** of its output has no non-zero exit code; a command run several times counts by its last run ("the same command" is the matched segment — a different `cd` prefix, redirection or `&& echo PASS` decoration does not make it a new one); click a cell to zoom to the decisive command and open its details. Artifacts = files touched by successful write/edit/patch calls, deduplicated (expand the list in the cell); task done = the last `turn/end` is not error/aborted/interrupted/blocked and the last step is an answer; the human check only asks whether a human message followed the final answer — content is never interpreted. With no verification command at all the badge reads a neutral "unverified"; code-mode sessions (real tool calls dispatched inside `run_code` scripts) are not recognized yet and the block says so. Rules live in `ANALYSIS_RULES.VALIDATION` in `verdict.js`.
