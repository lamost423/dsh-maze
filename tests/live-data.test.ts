...
describe('snapshotToMazeData', () => {
  ...
  it('carries what the outcome block needs: exit code off the raw last line, turn endings, human message times', () => {
    // 1.x 快照没有 turn/end 的 reason：turnEnds 只列已收尾的轮，配合 turn-error / turn-max-tokens 节点定原因
    const snap = {
      partial: null,
      turnEnds: new Map([[1, 99]]),
      turnTimings: new Map([[1, { startTime: t0, endTime: t0 + 8100 }]]),
      nodes: [
    ...
    expect(tools.find(t => t.callId === 'a')!.resFull!.length).toBe(5000)
  ...
  it('derives error / max-tokens endings from the nodes, completed from the snapshot turnEnds map', () => {
    const snap = {
      partial: null,
      turnEnds: new Map([[1, 12], [2, 22]]),
      turnTimings: new Map([[1, { startTime: t0, endTime: t0 + 2500 }], [2, { startTime: t0 + 10_000, endTime: t0 + 12_600 }]]),
      nodes: [
        { kind: 'user', seq: 1, time: t0 },
        { kind: 'assistant', seq: 11, time: t0 + 2000, timing: { stepStartTime: t0 + 1000 }, blocks: [{ kind: 'text', text: 'a' }] },
        ...
        { kind: 'turn-max-tokens', seq: 22, time: t0 + 12_500, turn: 2, step: 1 },
